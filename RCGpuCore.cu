...
// XOR-fold jump index hash (Phase 2B: Miller-Venkatesan spectral optimization)
// Replaces simple x[0] % JMP_CNT to reduce anticollisions by using all bits of x
// Cost: 7 XOR + 4 shift (single cycle on sm_86+), much better mixing than low-bits-only
...
__global__ void KernelA(const TKparams Kparams)
{
    ...
    for (int step_ind = 0; step_ind < STEP_CNT; step_ind++)
    {
        ...
        for (int group = PNT_GROUP_CNT - 1; group >= 0; group--)
        {
			...
			{
#ifdef GS_MODE
				__align__(16) u64 cx_next[4];
				CanonicalX(cx_next, x);
				u32 jmp_next = JumpHash(cx_next);
#else
...
#ifdef GS_MODE
			// GS mode: canonicalize once, check canonical x for DP
			// This inherently gives 3x DP rate (checking min of x, bx, b2x)
			{
				__align__(16) u64 canon_x[4];
				u32 equiv_class = Canonicalize(canon_x, x, y);
				if ((canon_x[3] & dp_mask64) == 0)
				{
					...
					dst[0] = ((int4*)canon_x)[0]; // store canonical x
					jmp_ind |= DP_FLAG;
				}
			}
#else
